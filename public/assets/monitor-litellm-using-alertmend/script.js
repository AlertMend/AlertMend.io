    ...
    signupForm.addEventListener('submit', async (e) => {
      ...
      try {
        const response = await fetch('https://api.alertmend.io/contact', {
          ...
          body: JSON.stringify({ full_name: 'Blog subscriber', company: '', email, message: 'Blog signup', source: 'blog_signup' }),
        });
        if (response.ok && input) { input.value = ''; if (signupStatus) { signupStatus.hidden = false; signupStatus.textContent = "Thanks! You're on the list."; signupStatus.className = 'signup-status success'; } }
      } catch { /* ignore */ }
