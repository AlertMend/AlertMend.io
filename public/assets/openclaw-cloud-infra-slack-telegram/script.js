      ...
      try {
        const response = await fetch('https://api.alertmend.io/contact', {
          ...
          body: JSON.stringify({
            ...
            company: '',
            email,
            message: 'Newsletter signup from the AlertMend blog.',
            source: 'blog_signup',
          }),
        });
        ...
        if (signupStatus) {
          ...
          signupStatus.textContent = response.ok ? "Thanks! You're on the list." : 'Something went wrong.';
          signupStatus.className = 'signup-status ' + (response.ok ? 'success' : 'error');
        }
      } catch {
        if (signupStatus) {
          signupStatus.hidden = false;
