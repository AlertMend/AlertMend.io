...
export const metadata = {
  ...
  keywords:
    'Elasticsearch cluster failure troubleshooting, fix Elasticsearch instability, Elasticsearch node failure recovery, troubleshoot Elasticsearch red yellow cluster, Elasticsearch shard allocation issues fix, optimize Elasticsearch cluster performance, Elasticsearch JVM heap size tuning, Elasticsearch resource allocation best practices, Elasticsearch network connectivity issues, fix unassigned shards in Elasticsearch, Elasticsearch cluster health monitoring, Elasticsearch high memory usage solution, Elasticsearch scaling guide, Elasticsearch replica and shard configuration, prevent Elasticsearch cluster split brain, Elasticsearch performance optimization tips, fix Elasticsearch node crash, Elasticsearch cluster recovery best practices, monitor Elasticsearch node stats, Elasticsearch cluster maintenance guide',
  openGraph: {
    images: [blog.image],
  },
  twitter: {
  ...
  alternates: {
    canonical: 'https://www.alertmend.io/blogs/Troubleshooting-Elasticsearch-Cluster-Failures-and-Instability.html',
  },
};
