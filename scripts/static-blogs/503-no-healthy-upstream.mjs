...
export async function build(slug) {
  ...
  const excerpt =
    meta.excerpt ||
    'No healthy upstream means zero backends passed health checks. Use our decision tree and playbooks for nginx, Envoy, Istio, HAProxy, and ALB to fix it fast.'
    ...
    <div class="dl-blog">
      <section class="heroBand fearBand">
        <div class="heroBrands">
          <span class="heroBrandText" style="font-size:1.5rem;font-weight:700;color:#09090b;letter-spacing:-0.02em">503 No Healthy Upstream</span>
        ...
        <p class="heroGuideLabel">Kubernetes &amp; reverse proxy runbook · upstream health</p>
        <p class="fearHeadline">HTTP 503. MESSAGE: no healthy upstream. ENDPOINTS: &lt;none&gt;.</p>
        <p class="fearLead">The proxy is fine. Every backend failed health checks or was never Ready. Teams restart nginx, scale replicas, and delete pods while <code>kubectl get endpoints</code> still shows empty. The root cause is not where they look first: five distinct failure modes produce the same 503, and each needs a different fix.</p>
      ...
      <section class="heroBand heroBandCompact">
        <p class="seoTldr"><strong>TL;DR:</strong> <strong>No healthy upstream</strong> means zero backends passed health checks — but the root cause splits five ways, and the wrong first move makes recovery slower. Below: the <a href="#diagnostic-tree">diagnostic decision tree</a>, <a href="#failure-playbook">failure-mode playbooks</a> for nginx, Envoy, Istio, HAProxy, and ALB, plus <a href="#faq">FAQ</a> for every error-text variant.</p>
      </section>
      ...
      <p class="bodyText"><strong>No healthy upstream</strong> means the reverse proxy or load balancer received your request but has zero backend servers passing health checks. The proxy itself is running — every upstream host is down, failing probes, or ejected by the service mesh.</p>
      ...
      <figure class="flowDiagram">
        <img src="${flowImage}" alt="Request flow: client to ingress to empty upstream pool returning 503" width="960" height="200" loading="lazy">
        <figcaption class="flowDiagramCaption">The proxy works. The upstream pool is empty or every host failed health checks.</figcaption>
      </figure>
      <div class="diyWrap">
        <table class="compareTable">
      ...
      <p class="verifiedNote">Commands verified on Kubernetes 1.30 and ingress-nginx.</p>
...
kubectl get svc &lt;service-name&gt; -n &lt;namespace&gt; -o yaml | grep -A5 selector</pre>
      ...
      <p class="bodyText">External URL checks catch 503 before your users do. AlertMend correlates failures with endpoint drains and recent rollouts.</p>
      <div class="amFlow">
      ...
      <div class="ctaBand">
        <div class="ctaBandTitle">Stop debugging 503 from scratch every time.</div>
        <p class="ctaBandSub">You ran endpoints checks, read probe Events, and walked five failure-mode playbooks. AlertMend automates that workflow: URL monitoring on your ingress, Slack alerts with endpoint context, and safe rollout undo when a deploy drains every backend. Page humans only when auto-remediation fails.</p>
        <div class="ctaBtnRow">
