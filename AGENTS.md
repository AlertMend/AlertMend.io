# AGENTS.md — AlertMend.io build & rendering guide
...
&& npm run build:blog         # 3. emit dist/blog/<slug>/index.html
...
| 3    | `new_blogs/*.md`                               | `dist/blog/<slug>/index.html` (one per post, prerendered)  |
...
**Output:** `dist/blog/<slug>/index.html` (served at `/blog/<slug>`)

A blog post lives in two places during the build:
- `new_blogs/<slug>.md` — the canonical markdown source
- `dist/blog/<slug>/index.html` — a hand-templated, SEO-optimised standalone
  HTML page generated by `marked` + a string template inside
  `build-blog-html.js`

The HTML build uses its own page chrome (header / footer / canonical
link) and is what Google primarily ranks for `/blog/<slug>` URLs.
...
Production exposes one canonical URL per article: `/blog/<slug>`.
The generated HTML owns that canonical URL; the React route provides
the development and client-side navigation path for the same article.
