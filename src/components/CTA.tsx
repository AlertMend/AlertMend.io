...
export default function CTA({ solutionId = 'default' }: CTAProps) {
  ...
  return (
    <section id="cta" className="py-20 md:py-24 px-4 sm:px-6 lg:px-8 text-brand-800 relative overflow-hidden gradient-bg-white">
      {/* Background decoration */}
      <div className="absolute inset-0 overflow-hidden">
        <div className="absolute top-1/4 -left-20 w-96 h-96 bg-brand-50/30 rounded-full blur-3xl animate-pulse"></div>
        <div className="absolute bottom-1/4 -right-20 w-96 h-96 bg-brand-50/25 rounded-full blur-3xl animate-pulse animate-delay-1s"></div>
        <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-brand-50/20 rounded-full blur-3xl animate-pulse animate-delay-2s"></div>
      </div>
      
      <div className="max-w-5xl mx-auto text-center relative z-10">
        <div className="inline-block px-5 py-2 bg-gradient-to-r from-brand-50 to-brand-100 text-brand-700 rounded-full text-sm font-bold mb-5 shadow-md border border-brand-200/50">
          Get Started
        </div>
        <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold mb-5 leading-tight text-brand-900">
          Ready to Transform Your Infrastructure Operations?
        </h2>
        <p className="text-lg md:text-xl text-brand-700 mb-8 max-w-3xl mx-auto leading-relaxed">
        ...
        <div className="flex flex-col sm:flex-row gap-3 justify-center mb-8">
          ...
          <button 
            ...
            className="border-2 border-brand-800 text-brand-900 px-10 py-4 rounded-xl font-semibold text-lg hover:bg-brand-50 hover:border-brand-900 transition-all"
          >
            Start Free Trial
          </button>
        </div>

        <div className="flex flex-wrap justify-center gap-8 text-base text-brand-700">
